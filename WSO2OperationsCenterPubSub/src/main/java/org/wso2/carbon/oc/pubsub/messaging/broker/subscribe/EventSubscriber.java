...
package org.wso2.carbon.oc.pubsub.messaging.broker.subscribe;

import org.wso2.carbon.oc.pubsub.messaging.broker.connect.RetryTimer;
import org.wso2.carbon.oc.pubsub.messaging.broker.connect.TopicSubscriberFactory;
...
public class EventSubscriber implements Runnable {
    ...
    private static final Log log = LogFactory.getLog(
		    org.wso2.carbon.oc.pubsub.messaging.broker.subscribe.EventSubscriber.class);
    private final org.wso2.carbon.oc.pubsub.messaging.broker.connect.TopicSubscriber topicSubscriber;
    ...
    public EventSubscriber(String topicName, MessageListener messageListener) {
        ...
        this.topicSubscriber = TopicSubscriberFactory
		        .createTopicSubscriber(protocol, messageListener, topicName);

        if (log.isDebugEnabled()) {
            log.debug(String.format("Topic subscriber created: [protocol] %s [topic] %s", protocol,
                                    topicName));
        }
    }
