...
import org.wso2.carbon.oc.pubsub.messaging.broker.connect.TopicPublisherFactory;
...
public class EventPublisher {
    ...
    private static final Log log = LogFactory.getLog(
		    org.wso2.carbon.oc.pubsub.messaging.broker.publish.EventPublisher.class);
    ...
    EventPublisher(String topicName) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Topic publisher created: [protocol] %s [topic] %s", protocol,
                                    topicName));
        }
    }
    ...
    public void publish(Object messageObj, boolean retry) {
        synchronized (org.wso2.carbon.oc.pubsub.messaging.broker.publish.EventPublisher.class) {
            Gson gson = new Gson();
